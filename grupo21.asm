...
IMG_ROVER_NO_ENERGY		EQU 4		; imagem para quando o rover fica sem energia
...
processo_controlo:
	...
	MOV [APAGA_ECRAS], R10				; apaga os pixels do ecrã
	MOV [REMOVE_BG], R10				; apaga o aviso de nenhum cenário selecionado
	MOV [REMOVE_FG], R10				; remove a imagem do foreground
...
controlo_teclado:
	...
	MOV R9, [estado_do_jogo]			; estado do jogo de jogo
	...
	JZ controlo_tecla_parar				; se for, tenta controlo_tecla_parar o jogo
	JMP controlo_teclado
controlo_tecla_comecar:
	CMP R10, JOGO_ATIVO					; verifica se o jogo está ativo
	JZ controlo_teclado					; se estiver, não faz nada
	MOV R10, JOGO_ATIVO
	MOV [estado_do_jogo], R10			; define o estado do jogo como ativo
	...
	MOV [REMOVE_FG], R10				; remove o cenário frontal
...
controlo_tecla_pausar:
	CMP R10, JOGO_PARADO				; verifica se o jogo está parado
	JZ controlo_teclado					; se estiver, não faz nada
	CMP R10, JOGO_EM_PAUSA				; verifica se o jogo está em pausa
	JZ controlo_resumir					; se estiver, recomeça o jogo
	MOV R10, JOGO_EM_PAUSA
	MOV [estado_do_jogo], R10			; define o estado do jogo como em pausa
	MOV [SELECIONA_FG], R6				; seleciona a imagem de pausa no foreground
	MOV [PAUSA_MIDIAS], R10				; pausa todos os sons/vídeos em reprodução
	JMP controlo_teclado
controlo_resumir:
	MOV R10, JOGO_ATIVO
	MOV [estado_do_jogo], R10			; define o estado do jogo como ativo
	MOV [RETOMA_MIDIAS], R10			; retoma todos os sons/vídeos em pausa
	MOV [REMOVE_FG], R10				; remove a imagem do foreground
	MOV [evento_comecar], R6			; ativa o jogo (desbloqueia os respetivos processos)
	JMP controlo_teclado				; reinicia
controlo_tecla_parar:
	CMP R10, JOGO_ATIVO					; verifica se o jogo já está ativo
	JNZ controlo_teclado				; se não estiver, não faz nada
	MOV R10, JOGO_PARADO
	MOV [estado_do_jogo], R10			; define o estado do jogo como parado
	MOV [APAGA_ECRAS], R10				; apaga todos os pixels já desenhados (o valor de R0 não é relevante)
	MOV [REMOVE_FG], R10				; remove a imagem do cenário de fundo
	MOV [TERMINA_MIDIAS], R10
...
avanca_missil:
	...
	JZ avanca_missil_fim		; se o míssil não tiver mais movimentos para fazer, termina a rotina
...
cria_missil:
	...
	MOV R10, [coluna_rover]			; coluna de referência do rover
	...
	MOV R8, missil					; tabela que define o míssil
	CALL desenha_boneco				; desenha o míssil
...
colisao_met_bom_rover:
	...
	MOV [REPRODUZ_MIDIA], R3		; toca o som
	CALL adiciona_energia			; atualiza a energia do rover
